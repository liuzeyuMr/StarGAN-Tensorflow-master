...
class StarGAN(object) :
    def __init__(self, sess, args):
        ...
        self.augment_flag = args.augment_flag #是否使用图像增强

        self.epoch = args.epoch #20
        self.iteration = args.iteration #10000
        self.decay_flag = args.decay_flag #ture
        self.decay_epoch = args.decay_epoch #10

        self.gan_type = args.gan_type #wan_gp

        self.batch_size = args.batch_size #16
        self.print_freq = args.print_freq #1000
        self.save_freq = args.save_freq   #1000

        self.init_lr = args.lr #0.0001
        self.ch = args.ch  #64
        self.selected_attrs = args.selected_attrs  #['Black_Hair', 'Blond_Hair', 'Brown_Hair', 'Male', 'Young']
        # print(self.selected_attrs)
        self.custom_label = np.expand_dims(args.custom_label, axis=0) # [1,5]
        # print(self.custom_label.shape)
        self.c_dim = len(self.selected_attrs) #5

        """ Weight """
        self.adv_weight = args.adv_weight #1
        self.rec_weight = args.rec_weight #10
        self.cls_weight = args.cls_weight #10
        self.ld = args.ld #10

        """ Generator """
        self.n_res = args.n_res #6

        """ Discriminator """
        self.n_dis = args.n_dis #6
        self.n_critic = args.n_critic #5

        self.img_size = args.img_size #128
        self.img_ch = args.img_ch #3

        print()

        print("##### Information #####")
        print("# 使用的Gan的损失函数 : ", self.gan_type)
        print("# 选择转换的属性 : ", self.selected_attrs)
        print("# 数据集 : ", self.dataset_name)
    ...
    def build_model(self):
        ...
        Image_data_class.preprocess() #完成数据处理部分
        ...
        x_fake = self.generator(self.x_real, label_trg) # real a +target domain
        x_recon = self.generator(x_fake, label_org, reuse=True) # real b 重构回去origin domain
        ...
        g_cls_loss = classification_loss(logit=fake_cls, label=label_trg) #让假的图片判别出是出自目标领域的
        g_rec_loss = L1_loss(self.x_real, x_recon) #计算real 和 recon的L1_loss
