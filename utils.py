...
class ImageData:
    ...
    def __init__(self, load_size, channels, data_path, selected_attrs, augment_flag=False):
        ...
        self.lines = open(os.path.join(data_path, 'list_attr_celeba.txt'), 'r').readlines() # 一次读取
    ...
    def preprocess(self) :
        all_attr_names = self.lines[1].split() #得到40个属性
        # print("11111111",all_attr_names)
        for i, attr_name in enumerate(all_attr_names) :
            self.attr2idx[attr_name] = i #字典保存属性index[]
            self.idx2attr[i] = attr_name #字典保存属性


        lines = self.lines[2:] #获得所有的label
        random.seed(1234)
        random.shuffle(lines) #打乱

        for i, line in enumerate(lines) :
            split = line.split()
            filename = os.path.join(self.data_path, split[0]) #获得文件名
            values = split[1:] #文件名对应的label 是一个【1,40】的列表，表示40个属性的状态

            label = []

            for attr_name in self.selected_attrs : #5
                idx = self.attr2idx[attr_name] #获得属性对应的index

                if values[idx] == '1' :
                    label.append(1.0)
                else :
                    label.append(0.0) #这个label是一个[1,5]列表
...
def create_labels(c_org, selected_attrs=None):
    ...
    # print("!!!!!!!!!!",hair_color_indices) 【0,1,2】

    c_trg_list = []

    for i in range(len(selected_attrs)): #5个循环 0 1 2 3 4
        c_trg = c_org.copy() #属性列表【2000,5】

        if i in hair_color_indices:  # 0 1 2 Set one hair color to 1 and the rest to 0.
            c_trg[:, i] = 1.0
            for j in hair_color_indices: # 0 1 2
                if j != i:
                    c_trg[:, j] = 0.0
        else:
